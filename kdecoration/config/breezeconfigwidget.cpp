...
ConfigWidget::ConfigWidget(QObject *parent, const KPluginMetaData &data, const QVariantList & /*args*/)
    ...
    , m_configuration(KSharedConfig::openConfig(QStringLiteral("vibrantizerc")))
    , m_changed(false)
{
    // configuration
    m_ui.setupUi(widget());
...
void ConfigWidget::save()
{
    ...
    {
        QDBusMessage message(QDBusMessage::createSignal("/VibrantizeDecoration", "org.kde.Vibrantize.Style", "reparseConfiguration"));
        QDBusConnection::sessionBus().send(message);
    }
}
